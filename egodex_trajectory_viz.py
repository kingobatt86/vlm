...
def smooth_positions(positions, window_size=5):
    ...
    for i in range(3):  # x, y, z
        smoothed[:, i] = uniform_filter1d(positions[:, i], size=window_size, mode='nearest')
    return smoothed


def smooth_rotations(rotation_matrices, window_size=5):
    """
    使用球面线性插值(SLERP)平滑旋转轨迹
    rotation_matrices: list of (3,3) rotation matrices
    window_size: 平滑窗口大小
    """
    ...
    for i in range(n):
        ...
        avg_quat = np.mean(quats[start:end], axis=0)
        avg_quat = avg_quat / (np.linalg.norm(avg_quat) + 1e-8)
        smoothed_quats[i] = avg_quat
    
    # 转换回旋转矩阵
    smoothed_matrices = []
    for q in smoothed_quats:
        try:
            r = Rotation.from_quat(q)
            smoothed_matrices.append(r.as_matrix())
        except:
            smoothed_matrices.append(np.eye(3))
...
# ===========================
def main(args):
    rr.init("egodex_trajectory_visualizer", spawn=True)
    ...
    for side in ['left', 'right']:
        print(f"Extracting {side} hand trajectory...")
        ...
        # 平滑处理
        print(f"  Smoothing with window size {args.smooth_window}...")
        origins_smooth = smooth_positions(origins_world, window_size=args.smooth_window)
        rotations_smooth = smooth_rotations(rotations_world, window_size=args.smooth_window)
        
        # 转换到相机坐标系（使用第一帧的相机外参作为参考）
        # 或者可以对每帧使用对应的相机外参
        origins_cam_list = []
        rotations_cam_list = []
        ...
        for t in range(num_frames):
            ...
            # 位置转换
            origin_cam = (world_to_cam @ np.append(origins_smooth[t], 1.0))[:3]
            origins_cam_list.append(origin_cam)
            
            # 旋转转换
            R_cam = world_to_cam[:3, :3] @ rotations_smooth[t]
    ...
    output_file = f"egodex_trajectory_{args.episode_id}.rrd"
    rr.save(output_file)
    print(f"Done! Saved to {output_file}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="可视化机械臂末端轨迹")
